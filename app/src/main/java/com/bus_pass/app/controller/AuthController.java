...
<<<<<<< HEAD
    ...
    public ResponseEntity<java.util.Map<String, String>> extractAadhaar(@org.springframework.web.bind.annotation.RequestParam("file") org.springframework.web.multipart.MultipartFile file) {
        try {
            String aadhaarNumber = ocrService.extractAadhaarNumber(file);
            String masked = "XXXX XXXX " + (aadhaarNumber.length() >= 4 ? aadhaarNumber.substring(aadhaarNumber.length() - 4) : aadhaarNumber);
            String adharFileUrl = fileStorageUtil.storeFile(file);

            java.util.Map<String, String> response = new java.util.HashMap<>();
            response.put("aadhaarNumber", aadhaarNumber);
            response.put("masked", masked);
            response.put("adharFileUrl", adharFileUrl);
